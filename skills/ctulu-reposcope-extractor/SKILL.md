...
## Contexte
Le stage EXTRACT du pipeline REPOSCOPE-COMPARE. Extrait les dimensions sémantiques, techniques et architecturales d'un repo GitHub via l'API GitHub (gh CLI + httpx fallback).
...
| `--out` | `data/` | Répertoire de sortie pour Parquet |
| `--dry-run` | `false` | Simulation sans appel API |
| `--wal` | `false` | Ajout d'une entrée WAL |
...
- **Dérivé**: strate_L (inféré L0-L9), has_tests, deps_count
...
- Ne JAMAIS parser le README complet (limité à 500 chars)
