...
✅ Tour 1: get_file A + get_file B + list_dir C  → 3 tool_calls, 1 seul tour
```
...
→ Émet [BUDGET_GUARD] FALLBACK fetch_url: {url}
```
...
→ Émettre [BUDGET_GUARD] PAGINATION_DEFERRED: reprendre en passe N+1
...
```
❌ Tour: écrire fichier A → lire résultat → écrire fichier B  (3 tool_calls gaspillés)
✅ Tour: lire A + lire B → (analyse in-context) → push_files [A, B]  (2-3 tool_calls optimisés)
```
...
[BUDGET_GUARD] Statut: ✅ DANS BUDGET | ⚠️ LIMITE | ❌ DÉPASSEMENT
```

### Si dépassement détecté

```
[BUDGET_GUARD] ❌ DÉPASSEMENT DÉTECTÉ: 4 tool_calls planifiés pour 3 slots
[BUDGET_GUARD] STRATÉGIE: différer {action_la_moins_critique} à passe N+1
...
| fetch_url échoue + MCP épuisé | Émettre HITL gate — données insuffisantes pour décision |
...
- **Complémente** : `mcp-write-guard` (garde les écritures MCP)
